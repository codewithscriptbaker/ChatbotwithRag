...
const REGENERATE_CHUNK_DELAY_MS = 18
...
interface UseChatSessionReturn {
  ...
  handleRegenerateAssistant: (assistantMessageId: string) => Promise<boolean>
}
...
export function useChatSession(chatId: string): UseChatSessionReturn {
  ...
  useEffect(() => {
    const handleAppendInline = (event: Event) => {
      const { messages: newMessages } = (event as CustomEvent<{ messages: ChatMessage[] }>).detail
      if (!Array.isArray(newMessages) || newMessages.length === 0) return

      setMessages((prev) => {
        const next = [...prev, ...newMessages]
        commitConversation(chatId, next, { updateMeta: true })
        return next
      })
    }

    window.addEventListener('chat:append-inline-messages', handleAppendInline as EventListener)
    return () => {
      window.removeEventListener('chat:append-inline-messages', handleAppendInline as EventListener)
    }
  }, [chatId, setMessages])
  ...
  const handleRegenerateAssistant = useCallback(
    async (assistantMessageId: string): Promise<boolean> => {
      if (isLoading || regeneratingAssistantId !== null) return false
      ...
      const targetAssistantMessage = messages[assistantIndex]
      const originalTargetParts = targetAssistantMessage.parts
      ...
      try {
        ...
        setMessages((previousMessages) => {
          ...
          if (nextAssistantIndex < 0) return previousMessages
          nextMessages[nextAssistantIndex] = {
            ...nextMessages[nextAssistantIndex],
            parts: []
          }
          return nextMessages
        ...
        const regeneratedText = typeof data.text === 'string' ? data.text : ''
        const wordChunks = regeneratedText.match(/\S+\s*/g) ?? []

        setRegeneratingPhase('streaming')

        if (wordChunks.length > 0) {
          let accumulated = ''

          for (const chunk of wordChunks) {
            if (regenerateTokenRef.current !== currentToken) {
              return false
            }
            ...
            await new Promise((resolve) => {
              window.setTimeout(resolve, REGENERATE_CHUNK_DELAY_MS)
            })
          }
        }
        ...
        setMessages((previousMessages) => {
          ...
          nextMessages[nextAssistantIndex] = {
            ...
            parts: [{ type: 'text', text: regeneratedText }],
            createdAt: new Date()
          }

          commitConversation(chatId, nextMessages, { updateMeta: true })
          return nextMessages
        })
