...
const TEXTAREA_CLASS_NAME =
  'text-foreground w-full min-w-0 resize-none rounded-none border-0 bg-transparent text-[15px] leading-6 break-words outline-none shadow-none focus:outline-none focus-visible:ring-0 focus-visible:ring-offset-0 max-h-52 min-h-7 overflow-y-auto [field-sizing:content] md:text-[15px]'
...
export function ComposerTextarea({
  ...
  return (
    <div className="relative flex min-h-12 min-w-0 flex-1 items-start px-4 pt-3 pb-1">
      ...
      {showPlaceholder && !interimTranscript && (
        <span className="text-muted-foreground pointer-events-none absolute top-3 left-4 max-w-[calc(100%-2rem)] truncate text-[15px] leading-6">
          Ask anything
        </span>
      )}
      {interimTranscript ? (
        <span className="text-foreground pointer-events-none absolute top-3 left-4 text-[15px] leading-6">
          {message}
          <span
