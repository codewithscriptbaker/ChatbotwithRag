...
function RemoveAttachmentButton({
  ...
  return (
    <AppIconButton
      ...
      onClick={onClick}
      className={cn(
        'bg-background/95 text-muted-foreground hover:bg-background hover:text-foreground border-border/70 rounded-full border shadow-xs active:translate-y-0 transition-none',
        className
      )}
      aria-label={label}
    >
    ...
    <>
      <div className="flex flex-wrap gap-2 px-4 pt-3">
          ...
          <div key={img.id} className="group relative size-20">
            ...
            >
              <Image
                ...
                unoptimized
                sizes="80px"
                className="border-border/60 rounded-xl border object-cover shadow-xs transition-opacity group-hover:opacity-95"
                onError={() => onImagePreviewError(img.id, img.url)}
              />
            </button>
            <RemoveAttachmentButton
              onClick={() => onRemoveImage(img.id)}
              className="absolute -top-1.5 -right-1.5 size-6 opacity-100"
              label="Remove image"
            />
          </div>
        ))}
        {uploadedDocuments.map((doc) => {
          ...
          const isNote = doc.sourceType === 'note'

          return (
            <div
              key={doc.id}
              className="group border-border/60 bg-muted/35 relative flex items-center gap-2 rounded-xl border py-2 pr-8 pl-3 shadow-xs"
            >
              ...
              {isNote ? (
                <span className="text-muted-foreground ml-auto pr-1 text-xs">Note</span>
              ) : null}
              <RemoveAttachmentButton
                onClick={() => onRemoveDocument(doc.id)}
                className="absolute top-1/2 right-1 size-6 -translate-y-1/2 opacity-100"
