...
function UserMessage({ message }: MessageProps): React.JSX.Element {
  ...
  const userText = useMemo(() => getTextFromParts(message.parts), [message.parts])
  ...
  const handleEdit = useCallback(() => {
    ...
    window.dispatchEvent(
      new CustomEvent('chat:edit-user-message', {
        detail: {
          text: userText
        }
      })
  ...
  return (
    <div className="flex w-full justify-end">
      <div className="group flex max-w-[92%] min-w-0 flex-col items-end md:max-w-[88%]">
        ...
        <div className="mt-1 flex items-center gap-1 opacity-0 transition-opacity duration-150 group-hover:opacity-100 group-focus-within:opacity-100">
          <ButtonWithTooltip label={copied ? 'Copied' : 'Copy'}>
            <AppIconButton
              ...
              disabled={copied}
              onClick={handleCopy}
              aria-label={copied ? 'Message copied to clipboard' : 'Copy question'}
            >
              {copied ? <Check aria-hidden="true" /> : <Copy aria-hidden="true" />}
            </AppIconButton>
          </ButtonWithTooltip>
          <ButtonWithTooltip label="Edit">
            <AppIconButton
              variant="ghost"
              size="icon-sm"
              touch={false}
          ...
          <CopyStatusAnnouncement copied={copied} message="Question copied to clipboard." />
        </div>
      </div>
    </div>
  )
  ...
  return (
    <>
      <div className="mt-1 flex w-full max-w-full items-center">
        ...
        <ButtonWithTooltip label="Regenerate">
          <AppIconButton
            variant="ghost"
            size="icon-sm"
            touch={false}
...
function AssistantMessage({ message, isThinking, streamPhase }: MessageProps): React.JSX.Element {
  const { onRegenerateAssistant } = useChatMessagesContext()
  ...
  }, [message.id, onRegenerateAssistant])
